  ...
  return (
    <div className="p-4 md:p-8 space-y-4 md:space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-gray-100">Havuz</h1>
        ...
        <div className="flex flex-wrap gap-1">
          {["Tümü", ...CATEGORY_OPTIONS].map(c => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-colors ${catFilter === c ? "bg-indigo-600 text-white" : "bg-gray-900 border border-gray-800 text-gray-400 hover:border-gray-600"}`}
            >
              {c}
            </button>
          ))}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-3">
          {filtered.map(t => (
            <div key={t.id} className="bg-gray-900 border border-gray-800 rounded-2xl p-4 md:p-5 hover:border-gray-700 transition-all group">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="font-semibold text-gray-100 text-sm md:text-base leading-snug">{t.subject}</p>
                      <p className="text-xs md:text-sm text-gray-500 mt-0.5 truncate">{t.fromName || t.fromEmail}</p>
                    </div>
                  ...
                  <div className="flex items-center gap-2 mt-3 flex-wrap">
                    ...
                    <button
                      ...
                      className="ml-auto px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-lg transition-colors"
                    >
                      Ata
                    </button>
                  </div>
